...
const Cours = () => {
  ...
  const matieresStatic = niveauFilter && activitiesData[niveauFilter]
    ? Object.keys(activitiesData[niveauFilter])
    : [];
  ...
  const unitesStatic =
    niveauFilter && matiereFilter && activitiesData[niveauFilter] && activitiesData[niveauFilter][matiereFilter]
      ? Object.keys(activitiesData[niveauFilter][matiereFilter])
      : [];
  ...
  const titresStatic =
    ...
    activitiesData[niveauFilter] &&
    activitiesData[niveauFilter][matiereFilter] &&
    activitiesData[niveauFilter][matiereFilter][uniteFilter]
      ? activitiesData[niveauFilter][matiereFilter][uniteFilter]
      : [];
  ...
  // Professors
  const uniqueProfs = uniqueSorted(Activities.map(c => c.professeur));

  // Filter displayed items — use normalized compare to avoid spaces/case mismatch
  const displayedItems = Activities.filter(item => {
    return (
        ...
        <>
          ...
          <div className="filtersSection">
            ...
            >
              ...
              {[...new Set([
                ...Activities.map(c => c.niveau).filter(Boolean).map(s => s.trim()),
          ...
          <div className="itemsGrid">
            {displayedItems.length > 0 ? (
              ...
              <p className="noItemsText">Aucune activité trouvé pour ces filtres.</p>
            )}
          </div>
        </>
      )}
